...
All modules are called concurrently by the pipeline runner via
...
- ``datausa``    — population growth and income trends (Census ACS multi-year)
- ``serper``     — Google search signals (company presence, job postings, LinkedIn profile)
- ``edgar``      — SEC EDGAR public company detection (insight only, not scored)
